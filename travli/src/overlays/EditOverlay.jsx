...
function EditOverlay({activeTrip, closeEditOverlay}) {
    ...
    const handleSubmit = async (e) => {
        try {
            await axios.put('http://localhost:3001/trips', trip);
            alert("Trip edited!");
        } catch(error) {
            console.error('There was an error editing the trip!', error);
        }
    }
        ...
        <>
            <div className='window'>
                ...
                <form>
                    ...
                    <label>Start date:<input className="date-input" type="date" value={trip.startDate.slice(0, 10)} name="startDate"
                    style={startDateStyle} onChange={handleChange}/>
                    End date:<input className="date-input" type="date" value={trip.endDate.slice(0, 10)} name="endDate"
                    ...
                    <button type="button" onClick={handleCloseEditOverlay}>Cancel</button>
                    </div>
                </form>
            </div>
            <div className='overlay' onClick={handleCloseEditOverlay}/>
        </>
